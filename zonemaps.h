...
template <typename T> struct zone {
...
template <typename T> class zonemap {
  std::vector<T> elements; // vector/list that will hold all elements. Can
                           // convert to array for faster processing
  std::vector<zone<T>> zones;
  uint num_zones; // keeps track of the number of zones/partitions to the
                  // array/vector of elements
  uint num_elements_per_zone; // keeps track of number of elements per zone
                              // (items in every zone has to be less than or
                              // equal to this counter)
...
public:
  ...
  /**
   * Purpose: Query a range of keys using the zonemap that are between a lower
   * and higher limit Param: Lower and upper limit of range to be queried
