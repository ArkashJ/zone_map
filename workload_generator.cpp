...
void generate_one_file(unsigned long long pTOTAL_NUMBERS, unsigned int pdomain,
                       unsigned long long pL, short ppercent_outRange,
                       short plpercentage, int pseed);
std::string generate_partitions_stream(
    unsigned long long TOTAL_NUMBERS, unsigned int domain, unsigned long long L,
    short percent_outRange, short l_percentage, int seed, std::string folder);

void generate_one_file(unsigned long long pTOTAL_NUMBERS, unsigned int pdomain,
                       unsigned long long pL, short ppercent_outRange,
                       short plpercentage, int pseed = 1) {
  ...
  outfile << generate_partitions_stream(pTOTAL_NUMBERS, pdomain, pL,
                                        ppercent_outRange, plpercentage, pseed,
                                        folder_name)
          << std::endl;
...
unsigned int generate_random_in_range(unsigned long long position,
...
/*
    Function which generates uniform data over some domain, and write it in
   binary format. Each partition of L elements is shuffled, and has some noise
   (randomness) linked to the percent_outRange parameter.
    */
std::string generate_partitions_stream(unsigned long long TOTAL_NUMBERS,
                                       unsigned int domain,
                                       unsigned long long L,
                                       short percent_outRange,
                                       short l_percentage, int seed,
  ...
  for (unsigned long long i = 0; i < TOTAL_NUMBERS; i++) {
    ...
    if (ran < p_outOfRange && noise_counter < noise_limit) {
      ...
      unsigned long long r =
          generate_random_in_range(i, TOTAL_NUMBERS, l_percentage / 100.0);
...
// unsigned long long pTOTAL_NUMBERS, unsigned int pdomain, unsigned long long
// pL, short ppercent_outRange, int pseed

int main(int argc, char **argv) {
  if (argc < 4) {
    std::cout << "Usage: ./execs/workload_generator.exe totalNumbers "
                 "noisePercentage windowThreshold"
              << std::endl;
    return 0;
  }
  ...
  generate_one_file(totalNumbers, domain, windowSize, noisePercentage,
                    lPercentage, seedValue);
}
